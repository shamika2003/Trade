...
class Predictor:
    ...
    def _prepare_features(self, df):
        ...
        try:
            ...
            if X.columns.duplicated().any():
                log(f"ERROR | Duplicate columns found: {X.columns[X.columns.duplicated()]}")
                return None
            ...
            log(f"ERROR | Feature preparation failed: {e}")
            return None
    ...
    def _predict_model(self, model, X):
        ...
        value = np.clip(value, -0.01, 0.01)

        confidence = (0.7 + min(abs(value) * 20, 0.3))
    ...
    def predict( self, df, model_keys, symbol):
        ...
        try:
            ...
            signal_value, confidence = (self._predict_model(model, X))
